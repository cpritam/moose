...
InputParameters validParams<StressBasedChemicalPotential>()
{
  ...
  params.addRequiredParam<MaterialPropertyName>("property_name", "Name of stress based chemical potential");
  params.addRequiredParam<MaterialPropertyName>("stress_name", "Name of stress property variable");
  params.addRequiredParam<MaterialPropertyName>("direction_tensor_name", "Name of direction tensor variable");
    ...
    _has_coupled_c(isCoupled("c"))
{
  if (_has_coupled_c)
  {
    _dchemical_potential = &declarePropertyDerivative<Real>(getParam<MaterialPropertyName>("property_name"), getVar("c", 0)->name());
...
StressBasedChemicalPotential::computeQpProperties()
{
  ...
  _chemical_potential[_qp] = -_stress_old[_qp].doubleContraction(direction_tensor_rank_two) * _prefactor[_qp];

  if (_has_coupled_c)
    (*_dchemical_potential)[_qp] = -_stress_old[_qp].doubleContraction(direction_tensor_rank_two) * (*_dprefactor_dc)[_qp];
}
